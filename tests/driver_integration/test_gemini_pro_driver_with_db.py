...
class TestGeminiProDuomiWithDB(BaseVideoDriverTest):
    ...
    def test_build_create_request(self):
        ...
        # 验证 json 结构
        self.assertEqual(req['json']['model'], 'gemini-3-pro-image-preview')
        self.assertEqual(req['json']['prompt'], '测试提示词')
        self.assertEqual(req['json']['aspect_ratio'], '16:9')
        self.assertEqual(req['json']['image_urls'], ['https://example.com/test.jpg'])
        self.assertEqual(req['json']['image_size'], '2K')
    ...
    def test_build_check_query(self):
        """测试构建查询状态请求参数"""
        project_id = 'gemini_pro_test_task_123'
    ...
    def test_submit_task_success(self):
        ...
        with patch.object(self.driver, '_request') as mock_req:
            ...
            # 验证 json 参数
            self.assertEqual(call_args.kwargs['json']['model'], 'gemini-3-pro-image-preview')
            self.assertEqual(call_args.kwargs['json']['prompt'], '测试 Gemini Pro 提交成功')
            self.assertEqual(call_args.kwargs['json']['aspect_ratio'], '16:9')
            self.assertEqual(call_args.kwargs['json']['image_urls'], ['https://example.com/test.jpg'])
            self.assertEqual(call_args.kwargs['json']['image_size'], '1K')
    ...
    def test_submit_task_without_image_size(self):
        """测试提交任务 - image_size为空时使用默认值1K - mock _request"""
        task_id = self.create_test_ai_tool(
            ai_tool_type=GEMINI_PRO_IMAGE_TO_VIDEO_TYPE,
    ...
    def test_submit_task_with_multiple_images(self):
        ...
        with patch.object(self.driver, '_request') as mock_req:
            mock_req.return_value = {
                "code": 200,
                "msg": "success",
                "data": {"task_id": "gemini_pro_task_multi_789"}
            }
            ...
            # 验证 image_size 参数
            self.assertEqual(call_args.kwargs['json']['image_size'], '2K')
            
            self.assertTrue(result['success'])
            self.assertEqual(result['project_id'], 'gemini_pro_task_multi_789')
    ...
    def test_check_status_success(self):
        ...
        with patch.object(self.driver, '_request') as mock_req:
            ...
            # 验证调用参数
            self.assertIn('/api/gemini/nano-banana/gemini_pro_task_456', call_args.kwargs['url'])
    ...
    def test_check_status_failed(self):
        ...
        with patch.object(self.driver, '_request') as mock_req:
            ...
            # 验证调用参数
            self.assertIn('/api/gemini/nano-banana/gemini_pro_task_789', call_args.kwargs['url'])
            self.assertEqual(call_args.kwargs['method'], 'GET')
    ...
    def test_check_status_processing(self):
        ...
        with patch.object(self.driver, '_request') as mock_req:
            ...
            # 验证调用参数
            self.assertIn('/api/gemini/nano-banana/gemini_pro_task_999', call_args.kwargs['url'])
            self.assertEqual(call_args.kwargs['method'], 'GET')
