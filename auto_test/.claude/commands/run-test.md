...
测试清单中的每个步骤都有 `remark` 字段，用于记录测试过程中的细节和观察结果。
...
# 标记步骤为通过并添加备注
python test_navigator.py --pass-current --module <模块ID> --remark "登录界面显示正常，所有元素加载完成"

# 或指定功能和步骤
python test_navigator.py --pass node_005 1

# 指定功能和步骤并添加备注
python test_navigator.py --pass node_005 1 --remark "节点创建成功，界面响应正常"

# 或标记整个功能为通过
python test_navigator.py --pass node_005

# 标记整个功能为通过并添加备注
python test_navigator.py --pass node_005 --remark "所有步骤测试通过，功能正常"
```
